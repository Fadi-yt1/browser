...
export interface RunningBrowser {
  containerId: string;
  /** Where the orchestrator reaches the container's RFB port. */
  vncHost: string;
  vncPort: number;
  /** Where the orchestrator reaches the in-container control agent. */
  agentBase: string;
}
...
export async function launchBrowser(opts: LaunchOptions): Promise<RunningBrowser> {
  const { docker: d, screen } = config;
  const width = Math.min(Math.max(opts.width, 640), screen.maxWidth);
  const height = Math.min(Math.max(opts.height, 480), screen.maxHeight);
...
async function resolveEndpoint(container: Docker.Container): Promise<Omit<RunningBrowser, 'containerId'>> {
...
async function waitForReady(agentBase: string): Promise<void> {
  const deadline = Date.now() + config.docker.startupTimeoutMs;
...
export const dockerPing = async (): Promise<boolean> => {
  try {
    await docker.ping();
    return true;
  } catch {
